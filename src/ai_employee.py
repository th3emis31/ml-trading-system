...
def run_employee(force: bool = False, runner: Optional[Callable[[str], dict]] = None, now: Optional[datetime] = None,
                 base: Path = EMPLOYEE_DIR, root: Path = ROOT) -> dict:
    now = now or datetime.now(timezone.utc)
    paths = _paths(base)
    ...
    try:
        ...
        reply = (runner or run_claude_code)(employee_prompt(context))
...
def main(argv: Optional[list] = None) -> int:
    ...
    sub.add_parser("context", help="build and save the context only (no Claude call)")
    ...
    if args.command == "context":
        ...
        print(json.dumps({"sources": context["sources"], "prompt_characters": len(employee_prompt(context))}, indent=1))
        return 0
